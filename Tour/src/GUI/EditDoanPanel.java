 ...
 * @author Nam
 */
public class EditDoanPanel extends javax.swing.JPanel {
    ...
    static int idNguoidi;
    public static int currentIddoan;
    /**
     * Creates new form EmployeePanel
     */
    public EditDoanPanel() {
        initComponents();
        init();
    }
    ...
    public void setInfo()
    {
        ...
        {
            if(a.getDoan_id() == currentIddoan)
            {
                tendoanTxt.setText(a.getDoan_ten());
                idlb.setText(String.valueOf(currentIddoan));
                ngaydiTxt.setText(a.getDoan_ngaydi());
                ngayveTxt.setText(a.getDoan_ngayve());
                chitietTxt.setText(a.getDoan_chitiet());
                break;
            }
        }
        ...
        for(String a : arrNhanvienProp)
        {
            ...
            row.add(a.split("-")[0]);
            row.add(getTennhanvien(Integer.parseInt(a.split("-")[0])));
            row.add(a.split("-")[1]);
        ...
        for(String a : arrKhachProp)
        {
            Vector row = new Vector();
            row.add(a);
            row.add(getTenkhach(Integer.parseInt(a)));
        ...
        for(DoanDTO a : DoanBLL.sumArrDoan)
        {
            if(a.getDoan_id() == currentIddoan)
            {
                for(TourDTO t : tourbll.docdsdd())//cbb tour
                {
                    if(t.getTour_id() == a.getTour_id())
                    {
                        chontourCbbox.setSelectedItem(t.getTour_id() + "." + t.getTour_ten());
                        break;
                    }
                }
                setCbbgia();
                for(GiaTourDTO t : giatourbll.docdsgia())//cbb gia
                {
                    if(t.getGia_id()== a.getGia_id())
                    {
                        chongiaCbbox.setSelectedItem(t.getGia_id()+ "-" + t.getGia_sotien());
                        break;
                    }
                }
                break;
    ...
    public String getTenkhach(int id)
    {
        String str = "";
        khbll.docKhach();
    ...
    public void suaDoan()
    {
        if(check())
        {
            ...
            //them doan
            doandto = new DoanDTO(currentIddoan, idTour , idGia, tendoanTxt.getText(), ngaydiTxt.getText(), ngayveTxt.getText(),
                    chitietTxt.getText());
            bll.suaDoan(doandto);
            //them nguoi di
            nguoididto = new NguoidiDTO(getDsnhanvien(), getDskhachhang(), currentIddoan);
            bll.suaNguoidi(nguoididto);
            
            JOptionPane.showMessageDialog(null, "Sửa đoàn đi tour thành công");
...
class AddKhach extends AbstractCellEditor implements TableCellRenderer,TableCellEditor,ActionListener{
    ...
    private void initComponents() {
        ...
        jLabel2.setText("ĐOÀN ID:");
        infoPanel.add(jLabel2, new org.netbeans.lib.awtextra.AbsoluteConstraints(330, 10, -1, 30));

        idlb.setFont(new java.awt.Font("Times New Roman", 1, 16)); // NOI18N
        idlb.setForeground(new java.awt.Color(255, 0, 0));
        infoPanel.add(idlb, new org.netbeans.lib.awtextra.AbsoluteConstraints(400, 10, 70, 30));
